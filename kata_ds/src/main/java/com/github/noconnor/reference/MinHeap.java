...
public class MinHeap<E extends Comparable<? super E>> {
  ...
  public String toString() {
    ...
    reverseInOrder(1, builder, 0);
    return builder.toString();
  }

  private void reverseInOrder(int nodeIndex, StringBuilder builder, int level) {
    if (nodeIndex > 0 && nodeIndex <= size) {

      int nextLevel = level + 1;

      reverseInOrder(2 * nodeIndex + 1, builder, nextLevel);
      if (level == 0) {
        builder.append("(");
      ...
      reverseInOrder(2 * nodeIndex, builder, nextLevel);
